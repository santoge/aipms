"use client"

import { Button } from "@/components/ui/button"
import { Card, CardContent } from "@/components/ui/card"
import { Badge } from "@/components/ui/badge"
import {
  GraduationCap,
  Award,
  BookOpen,
  Users,
  Star,
  Heart,
  Brain,
  Activity,
  Phone,
  Mail,
  User,
  Calculator,
  Globe,
  Building,
} from "lucide-react"
import Image from "next/image"
import Header from "@/components/header"
import Footer from "@/components/footer"

const facultyMembers = [
  {
    name: "Thushara. A. J",
    position: "Principal",
    department: "Academic Leadership",
    image: "/leadership/thushara.jpg",
    qualifications: [
      "PhD Scholar in Biochemistry, University of Kerala",
      "MSc. Biochemistry - 2nd Rank Holder",
      "BSc. Biochemistry",
      "Former Research Assistant, University of Kerala",
    ],
    specialization: "Biochemistry & Academic Excellence",
    experience: "10+ Years",
    icon: GraduationCap,
    color: "from-indigo-500 to-blue-500",
  },
  {
    name: "Sreeshma O.S",
    position: "Vice Principal",
    department: "Nursing Education",
    image: "/faculty/sreeshma.jpg",
    qualifications: ["BSc Nursing", "MSc Nursing (Medical and Surgical Nursing)"],
    specialization: "Medical and Surgical Nursing",
    experience: "10 Years",
    icon: Heart,
    color: "from-red-500 to-pink-500",
  },
  {
    name: "Dr. Anjana S Raj",
    position: "Lecturer",
    department: "Anatomy & Physiology",
    image: "/faculty/anjana-new.jpg",
    qualifications: [],
    specialization: "Anatomy & Physiology",
    experience: "",
    icon: Brain,
    color: "from-purple-500 to-indigo-500",
  },
  {
    name: "Lijina A S",
    position: "Dialysis Lecturer",
    department: "Dialysis Technology",
    image: "/faculty/lijina.jpg",
    qualifications: ["BSC Dialysis Technology", "VHSE MLT"],
    specialization: "Dialysis Technology",
    experience: "",
    icon: Activity,
    color: "from-cyan-500 to-blue-500",
  },
  {
    name: "Tintymol Murali",
    position: "Lecturer",
    department: "Medical Laboratory Technology",
    image: "/faculty/tintymol.jpg",
    qualifications: ["Bsc. MLT"],
    specialization: "Medical Laboratory Technology",
    experience: "7 Years",
    icon: Calculator,
    color: "from-emerald-500 to-green-500",
  },
  {
    name: "Sujitha Surendran",
    position: "Lecturer",
    department: "Social Sciences",
    image: "/faculty/sujitha.jpg",
    qualifications: ["B.A History", "B.Ed Social Science", "K.TET (Category II)"],
    specialization: "History & Social Science Education",
    experience: "6 Years",
    icon: BookOpen,
    color: "from-green-500 to-emerald-500",
  },
  {
    name: "Renjini S R",
    position: "Computer Lecturer",
    department: "Computer Applications",
    image: "/faculty/renjini.jpg",
    qualifications: ["B.Com with Computer Application"],
    specialization: "Computer Applications & Technology",
    experience: "5 Years",
    icon: Calculator,
    color: "from-orange-500 to-red-500",
  },
  {
    name: "Aparna Gilbert A",
    position: "Lecturer",
    department: "Logistics",
    image: "/faculty/aparna.jpg",
    qualifications: ["BBA", "MBA (Logistics and Supply Chain Management)"],
    specialization: "Logistics & Supply Chain Management",
    experience: "8 Years",
    icon: Globe,
    color: "from-teal-500 to-cyan-500",
  },
  {
    name: "Sindhu Sre",
    position: "Receptionist",
    department: "Administration",
    image: "/faculty/sindhu.jpg",
    qualifications: ["Pre Degree"],
    specialization: "Student Services & Administrative Support",
    experience: "10 Years",
    icon: User,
    color: "from-pink-500 to-rose-500",
  },
]

const facultyQualities = [
  {
    icon: Award,
    title: "Highly Qualified",
    description: "Masters, Bachelor's degrees with specialized certifications",
    metric: "100%",
  },
  {
    icon: Heart,
    title: "Caring Attitude",
    description: "Friendly, caring and committed approach to student development",
    metric: "Always",
  },
  {
    icon: Brain,
    title: "Experienced",
    description: "Extensive experience in healthcare education and industry",
    metric: "5-10 Years",
  },
  {
    icon: Users,
    title: "Student-Focused",
    description: "Enthusiastic support and personalized attention to each student",
    metric: "Individual",
  },
]

const departments = [
  {
    name: "Academic Affairs",
    icon: GraduationCap,
    color: "from-blue-500 to-cyan-500",
    subjects: ["Nursing Education", "Academic Leadership", "Curriculum Development", "Student Assessment"],
  },
  {
    name: "Administration",
    icon: Building,
    color: "from-purple-500 to-pink-500",
    subjects: ["Educational Management", "Student Services", "Administrative Support", "Operations Management"],
  },
  {
    name: "Computer Applications",
    icon: Calculator,
    color: "from-green-500 to-emerald-500",
    subjects: ["Computer Applications", "Technology Integration", "Digital Systems", "IT Support"],
  },
  {
    name: "Logistics & Supply Chain",
    icon: Globe,
    color: "from-orange-500 to-red-500",
    subjects: ["Supply Chain Management", "Logistics Operations", "Inventory Management", "Transportation Systems"],
  },
]

export default function FacultyPage() {
  return (
    <div className="min-h-screen bg-white">
      <Header />

      {/* Hero Section */}
      <section className="relative py-24 bg-gradient-to-br from-[#001a4d] via-[#002366] to-[#003d82] text-white overflow-hidden">
        <div className="absolute inset-0">
          <div className="absolute inset-0 bg-gradient-to-br from-[#002366]/80 to-[#003d82]/70"></div>
        </div>

        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-5xl mx-auto text-center">
            <div className="mb-8">
              <span className="inline-flex items-center px-8 py-3 bg-white/10 backdrop-blur-md rounded-full text-sm font-medium text-blue-100 border border-white/20">
                <GraduationCap className="h-4 w-4 mr-2" />
                Expert Healthcare Educators
              </span>
            </div>

            <h1 className="text-5xl md:text-7xl font-bold mb-8 leading-tight">
              <span className="bg-gradient-to-r from-white via-blue-100 to-cyan-200 bg-clip-text text-transparent">
                Our
              </span>
              <br />
              <span className="text-white">Faculty</span>
            </h1>

            <p className="text-xl md:text-2xl mb-12 text-blue-100 max-w-4xl mx-auto leading-relaxed">
              Meet our{" "}
              <span className="text-cyan-300 font-semibold bg-cyan-300/10 px-2 py-1 rounded-lg border border-cyan-300/20">
                Distinguished Educators
              </span>{" "}
              committed to your academic and professional success
            </p>

            {/* Stats */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 max-w-4xl mx-auto">
              {[
                { number: "6", label: "Faculty Members" },
                { number: "8+", label: "Years Experience" },
                { number: "100%", label: "Committed Support" },
                { number: "Expert", label: "Industry Knowledge" },
              ].map((stat, index) => (
                <div key={index} className="text-center group">
                  <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10 hover:bg-white/10 transition-all duration-300">
                    <div className="text-3xl md:text-4xl font-bold text-cyan-300 mb-2">{stat.number}</div>
                    <div className="text-blue-200 text-sm font-medium">{stat.label}</div>
                  </div>
                </div>
              ))}
            </div>
          </div>
        </div>
      </section>

      {/* Faculty Overview */}
      <section className="py-20 bg-gradient-to-br from-gray-50 to-blue-50/30">
        <div className="container mx-auto px-4">
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-16">
              <div className="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-r from-[#002366] to-[#003d82] rounded-full mb-6">
                <Star className="h-8 w-8 text-white" />
              </div>
              <h2 className="text-4xl md:text-5xl font-bold text-[#002366] mb-6">Faculty Excellence</h2>
              <p className="text-xl text-gray-600 max-w-3xl mx-auto">
                Well qualified and experienced faculty with friendly, caring and committed attitude providing
                enthusiastic support
              </p>
            </div>

            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
              {facultyQualities.map((quality, index) => (
                <Card
                  key={index}
                  className="group hover:shadow-2xl transition-all duration-500 border-0 shadow-lg overflow-hidden bg-white hover:-translate-y-2"
                >
                  <CardContent className="p-8 text-center">
                    <div className="bg-gradient-to-r from-[#002366] to-[#003d82] rounded-full w-16 h-16 flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform duration-300">
                      <quality.icon className="h-8 w-8 text-white" />
                    </div>
                    <div className="text-3xl font-bold text-[#002366] mb-2">{quality.metric}</div>
                    <h3 className="text-xl font-semibold text-[#002366] mb-3">{quality.title}</h3>
                    <p className="text-gray-600 text-sm leading-relaxed">{quality.description}</p>
                  </CardContent>
                </Card>
              ))}
            </div>
          </div>
        </div>
      </section>

      {/* Faculty Members */}
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-16">
              <div className="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-r from-[#002366] to-[#003d82] rounded-full mb-6">
                <Users className="h-8 w-8 text-white" />
              </div>
              <h2 className="text-4xl md:text-5xl font-bold text-[#002366] mb-6">Meet Our Faculty</h2>
              <p className="text-xl text-gray-600 max-w-3xl mx-auto">
                Dedicated professionals shaping the future of healthcare education
              </p>
            </div>

            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {facultyMembers.map((faculty, index) => (
                <Card
                  key={index}
                  className="group hover:shadow-2xl transition-all duration-500 border-0 shadow-lg overflow-hidden bg-white hover:-translate-y-2"
                >
                  <CardContent className="p-0">
                    <div className="relative overflow-hidden">
                      <Image
                        src={faculty.image || "/placeholder.svg"}
                        alt={faculty.name}
                        width={400}
                        height={300}
                        className="w-full h-64 object-cover group-hover:scale-110 transition-transform duration-500"
                      />

                      {/* Overlay */}
                      <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>

                      {/* Department Badge */}
                      <Badge className="absolute top-4 left-4 bg-[#002366] text-white shadow-lg">
                        {faculty.department}
                      </Badge>

                      {/* Experience Badge */}
                      <Badge className="absolute top-4 right-4 bg-white/90 text-[#002366] shadow-lg">
                        {faculty.experience}
                      </Badge>

                      {/* Icon */}
                      <div
                        className={`absolute bottom-4 right-4 bg-gradient-to-r ${faculty.color} rounded-full p-3 shadow-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300`}
                      >
                        <faculty.icon className="h-6 w-6 text-white" />
                      </div>
                    </div>

                    <div className="p-6">
                      <h3 className="text-xl font-bold text-[#002366] mb-2 group-hover:text-[#003d82] transition-colors">
                        {faculty.name}
                      </h3>
                      <Badge className="bg-[#002366] text-white mb-4">{faculty.position}</Badge>
                      <p className="text-gray-600 mb-4 text-sm">{faculty.specialization}</p>

                      {/* Qualifications */}
                      <div className="mb-4">
                        <h4 className="font-semibold text-[#002366] mb-2 text-sm">Qualifications:</h4>
                        <div className="space-y-1">
                          {faculty.qualifications.map((qual, idx) => (
                            <div key={idx} className="flex items-center text-xs text-gray-600">
                              <Award className="h-3 w-3 text-green-500 mr-2 flex-shrink-0" />
                              {qual}
                            </div>
                          ))}
                        </div>
                      </div>
                    </div>
                  </CardContent>
                </Card>
              ))}
            </div>
          </div>
        </div>
      </section>

      {/* Departments */}
      <section className="py-20 bg-gradient-to-br from-gray-50 to-blue-50/30">
        <div className="container mx-auto px-4">
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-16">
              <div className="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-r from-[#002366] to-[#003d82] rounded-full mb-6">
                <BookOpen className="h-8 w-8 text-white" />
              </div>
              <h2 className="text-4xl md:text-5xl font-bold text-[#002366] mb-6">Academic Departments</h2>
              <p className="text-xl text-gray-600 max-w-3xl mx-auto">
                Specialized departments covering all aspects of healthcare education
              </p>
            </div>

            <div className="grid md:grid-cols-2 gap-8">
              {departments.map((dept, index) => (
                <Card
                  key={index}
                  className="hover:shadow-xl transition-all duration-300 border-0 shadow-lg bg-white overflow-hidden"
                >
                  <CardContent className="p-8">
                    <div className="flex items-center mb-6">
                      <div
                        className={`bg-gradient-to-r ${dept.color} rounded-full w-12 h-12 flex items-center justify-center mr-4`}
                      >
                        <dept.icon className="h-6 w-6 text-white" />
                      </div>
                      <h3 className="text-xl font-bold text-[#002366]">{dept.name}</h3>
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                      {dept.subjects.map((subject, idx) => (
                        <div key={idx} className="flex items-center">
                          <Activity className="h-4 w-4 text-green-500 mr-2 flex-shrink-0" />
                          <span className="text-gray-700 text-sm">{subject}</span>
                        </div>
                      ))}
                    </div>
                  </CardContent>
                </Card>
              ))}
            </div>
          </div>
        </div>
      </section>

      {/* CTA Section */}
      <section className="py-16 bg-gradient-to-r from-[#002366] to-[#003d82] text-white">
        <div className="container mx-auto px-4 text-center">
          <div className="max-w-3xl mx-auto">
            <div className="inline-flex items-center justify-center w-16 h-16 bg-white/10 rounded-full mb-6">
              <GraduationCap className="h-8 w-8 text-white" />
            </div>
            <h2 className="text-3xl md:text-4xl font-bold mb-4">Learn from the Best</h2>
            <p className="text-xl text-blue-100 mb-8">
              Join our academic community and learn from experienced healthcare professionals
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button size="lg" className="bg-white text-[#002366] hover:bg-blue-50">
                <Mail className="mr-2 h-5 w-5" />
                Contact Faculty
              </Button>
              <Button
                size="lg"
                variant="outline"
                className="border-white text-white hover:bg-white hover:text-[#002366] bg-transparent"
              >
                <Phone className="mr-2 h-5 w-5" />
                Schedule Meeting
              </Button>
            </div>
          </div>
        </div>
      </section>

      <Footer />
    </div>
  )
}
